...
DOCKER_GOPATH_BUILD ?= $(DOCKER) run --rm -v $(DIR):$(GOPATH)/src/$(GOTARGET) -w $(GOPATH)/src/$(GOTARGET) $(BUILD_IMAGE) /bin/sh -c
...
define E2E_OS_template
$(1)-$(2): bindir
	$(DOCKER_GOPATH_BUILD) 'GOPATH=$(GOPATH) GOARCH=amd64 GOOS=$(2) $(GO_TESTCMD) -o $(1)-$(2) ./test/$(3)'
ALL_BINS := $(ALL_BINS) $(1)-$(2)
endef

$(foreach cmd, $(COMMANDS), $(foreach os, $(OSES), $(eval $(call OS_template, $(cmd),$(os),$(notdir $(cmd))))))

$(foreach os, $(OSES), $(eval $(call E2E_OS_template, $(E2E_BINARY_TARGET),$(os),$(notdir $(E2E_BINARY_TARGET)))))
